...
-- Started on 2022-03-30 17:01:46 CEST
...
COPY public.comments (id, user_id, doc_id, description, date) FROM stdin;
\.
...
COPY public.document (id, title, source, description, "refDate", "publicationDate", "rightsHolder", file) FROM stdin;
1	mon titre	my source	ma description	2022-03-01 00:00:00+00	2022-01-11 00:00:00+00	my rights 	60ef72fe-fe31-4577-9ac5-489d0f69e59c.jpg
\.
...
COPY public.document_guided_tour (id, tour_id, doc_id, doc_position, text1, text2, title) FROM stdin;
\.
...
COPY public.document_user (id, doc_id, user_id) FROM stdin;
1	1	1
\.
...
COPY public.link_city_object (id, source_id, target_id, centroid_x, centroid_y, centroid_z) FROM stdin;
\.
...
COPY public."user" (id, username, password, "firstName", "lastName", email, role_id) FROM stdin;
1	admin	$pbkdf2-sha256$29000$zTnnXMv5P6fUem9trfXeOw$I95CgQcSsRwcT2bhgsxAp5PvBqCYvaukm9SzUvGGNEs	admin	UDViz	admin@udv.fr	1
\.
...
COPY public.user_role (id, label) FROM stdin;
1	admin
2	moderator
3	softModerator
4	contributor
...
COPY public.validation_status (doc_id, status) FROM stdin;
1	Validated
\.
...
COPY public.versions (id, doc_id, user_id, title, source, description, "refDate", "publicationDate", "rightsHolder", file, "quaternionX", "quaternionY", "quaternionZ", "quaternionW", "positionX", "positionY", "positionZ", version) FROM stdin;
\.
...
1	0.539966729531050516	0.249499656589288438	0.337179434174538162	0.72972315403247745	1845247.99563908321	5175150.34800419211	877.115114168263858
\.
...
SELECT pg_catalog.setval('public.document_user_id_seq', 1, true);
...
-- Completed on 2022-03-30 17:01:46 CEST
